$(document).ready(function() {
	...
	$(document).on('change', 'input[name^="PLANOMANSTTS___"]', function() {
        var name = $(this).attr('name');
        var index = name.split('___')[1];
        var valor = $(this).val();

        $('#PLANOMANSTATUS___' + index).val(valor);

        $('input[name="PLANOMANSTTS___' + index + '"]').each(function() {
	...
	$(document).on('blur', 'input[name^="PLANOMANSTTS___"]', function() {
        var name = $(this).attr('name');
        var index = name.split('___')[1];
        var valor = $(this).val();
...
function addLinhaTabela(tabela) {wdkAddChild(tabela);}
...
function deleteLinhaTabela(campo, tabela) {
	if (tabela != "TABELA_MAODEOBRA") {
        ...
        if ((campo.closest('tr').querySelectorAll('.form-group.col-md-2 input')[0].value) == "") {
            ...
            FLUIGC.toast({
                title: 'Atenção: ',
                message: 'Você não pode excluir um HHT que foi iniciado e pausado!',
                type: 'info'
            });
        }
    }
...
function finalizaOS(campo) {
    ...
    if (dataInicialStr != "" && document.getElementById('MAODEOBRAHORAFIM___' + index).value == "") {
        ...
        $('#MAODEOBRAHHT tr').each(function() {
            var nome = $(this).find('input[name^="MAODEOBRANOMEHHT"]').val();
            var horas = $(this).find('input[name^="MAODEOBRAHORASTOTAISHHT"]').val();
        ...
        var horasTotais = "00:00"
        $('#MAODEOBRAHHT tr').each(function() {
            var horas = $(this).find('input[name^="MAODEOBRAHORASTOTAISHHT___"]').val();
            if (horas != undefined) {
                horas = horas.split(":")[0] == "0" ? "0" + horas : horas;
                var momento1 = moment(horasTotais, 'HH:mm')
                var momento2 = moment(horas, 'HH:mm');				
...
function criarTabelaItensManutencao(IDPLANO) {
    ...
    try {
        ...
        for (var i = 0; i < dataset.values.length; i++) {
            ...
            var index = $('textarea[id^="PLANOMANATIVIDADE___"]:last').attr('id').split('___')[1];
            ...
            $('textarea[name="PLANOMANJUSTIFICA___' + index + '"]').each(function() {
                $(this)
                    .prop('disabled', true)
...
function consultaRMdaOS(tabela) {
    try {
        var c1 = DatasetFactory.createConstraint("IDMOVOS", document.getElementById('OSIDMOV').value, document.getElementById('OSIDMOV').value, ConstraintType.MUST);
...
async function cronogramaDeManutencao() {
    ...
    try {
        var dataset = await getDataset("ds_OSE_Cronograma_Preventivas", null, [DatasetFactory.createConstraint("NOME", local, local, ConstraintType.MUST)], null);
...
async function historicoPreventivas() {
    ...
    try {
        var dataset = await getDataset("ds_OSE_Historico_Preventivas", null, [DatasetFactory.createConstraint("NOME", local, local, ConstraintType.MUST)], null);
...
async function equipamentosSemPrev() {
    ...
    var local = document.getElementById("CRONOGRAMAOCAL").value;
    try {
        var dataset = await getDataset("ds_OSE_Equipamentos_Sem_Preventiva", null, [DatasetFactory.createConstraint("NOME", local, local, ConstraintType.MUST)], null);
